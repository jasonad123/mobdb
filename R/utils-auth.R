...
#'
#' @param refresh_token A string. Your Mobility Database API refresh token.
#'   Obtain this by signing up at https://mobilitydatabase.org and navigating
#'   to your account details page.
#' @param install A logical. If `TRUE`, will set the token in `.Renviron` for use
...
#' Get current Mobility Database API token, refreshing if necessary
#'
#' @description
#' Internal function to retrieve or refresh the access token. Users typically
#' don't need to call this directly.
#'
#' @param force A logical. If `TRUE`, force refresh the access token even if
#'   one already exists.
#'
...
#'
#' @param refresh_token A string. The refresh token.
#'
#' @return A string. A new access token.
#' @keywords internal
#' @noRd
