...
As of version 0.1.1, `mobdb` supports basic discovery of GBFS feeds through the `feeds()` function:

```{r gbfs-basic-discovery}
library(mobdb)

pdx <- feeds(provider = "portland", data_type = "gbfs")
pdx
# A tibble: 2 × 14
...
biketown <- feeds(provider = "biketown")
...
As of version 0.1.4, `mobdb` includes a number of convenience functions for discovery and parsing
of GBFS feeds 
...
nyc <- download_gbfs(provider = "Citi Bike")

# Load Citi Bike station details and status
nyc_stations <- get_gbfs_stations(nyc)
nyc_station_status <- get_gbfs_station_status(nyc)
