...
vignette: >
  %\VignetteIndexEntry{Introduction to mobdb}
...
**mobdb** is your first stop to analyzing transit in R. It helps you **find** and **download** GTFS feeds from the [Mobility Database](https://mobilitydatabase.org/), which contains information about 4000+ transit feeds worldwide.

After discovering and downloading feeds with mobdb, you can:

-   **Analyze** feeds with [tidytransit](https://r-transit.github.io/tidytransit/) - calculate frequencies, travel times, and visualize routes
-   **Edit** feeds with [gtfstools](https://ipeagit.github.io/gtfstools/) - filter, merge, and validate feeds
-   **Read/Write** feeds with [gtfsio](https://r-transit.github.io/gtfsio/) - fast I/O operations
...
bart <- mobdb_download_feed("mdb-53")

# Or search and download in one step (use full name for better results)
bart <- mobdb_download_feed(provider = "Bay Area Rapid Transit")
...
seattle_feeds <- mobdb_feeds(municipality = "Seattle", data_type = "gtfs")

# Find feeds in a country
canada_feeds <- mobdb_feeds(country_code = "CA", data_type = "gtfs")

# Combine filters
bc_feeds <- mobdb_feeds(
  country_code = "CA",
...
### Downloading with GTFS-Flex Filtering

By default, `mobdb_download_feed()` includes only core GTFS Schedule files. You can control GTFS-Flex inclusion:

```{r flex-filtering}
# Include all files (including Flex if present)
gtfs_with_flex <- mobdb_download_feed(
  provider = "Cape Cod",
  exclude_flex = FALSE
)

# Exclude Flex files (default)
gtfs_no_flex <- mobdb_download_feed(
  provider = "Cape Cod",
  exclude_flex = TRUE
)
```
...
feeds_list <- lapply(agencies, function(agency) {
  feeds <- mobdb_feeds(provider = agency, data_type = "gtfs")
  if (nrow(feeds) > 0) {
    mobdb_download_feed(feeds$id[1])
  }
})
...
translink <- mobdb_download_feed(vancouver_feeds$id[6])
...
feed_report <- vancouver_feeds$latest_dataset[6, ]
print(feed_report)
...
mobdb is just the first stop, not the end of the route when it comes to transit and transportation on R. 
Here are some other packages in the R ecosystem for analyzing GTFS.

-   [tidytransit](https://r-transit.github.io/tidytransit/): A tool to read and analyze GTFS feeds
-   [gtfstools](https://github.com/ipeaGIT/gtfstools): Edit and validate feeds
-   [gtfsio](https://github.com/r-transit/gtfsio): Fast I/O operations like saving GTFS back to ZIP files

Once you have GTFS files or GTFS objects, you can perform some pretty interesting analysis with other tools like:

-   [R5r](https://r-transit.github.io/tidytransit/): A wrapper for the R5 routing engine, to perform travel time and access analysis
-   [dodgr](https://github.com/UrbanAnalyst/dodgr): Distances on Directed Graphs in R