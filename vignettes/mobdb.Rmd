...
By default, `download_feed()` includes only core GTFS Schedule files. You can control GTFS-Flex inclusion:

```{r flex-filtering}
# Include all files (including Flex if present)
gtfs_with_flex <- download_feed(
  provider = "Cape Cod",
  exclude_flex = FALSE
)

# Exclude Flex files (default)
gtfs_no_flex <- download_feed(
  provider = "Cape Cod",
  exclude_flex = TRUE
)
```
...
translink <- download_feed(vancouver_feeds$id[6])
